...
// TODO 解决: 2. 如果用户长时间没有发送心跳包，那么就关闭conn
// TODO 重构了: 3. func (sandBox *SandBox) HandleExcuteCode(programPath, input _string) (_string, error)最后筛选答案可能有问题
// TODO 解决: 4. 在项目启动要确保容器已经存在并且正在运行
// TODO 解决: 5. 用户的输入都应该以"\n"结尾
// TODO 解决: 6. 统计每个算法题所用的时间目前是不准确的，这个时间应该要在c++代码层面去实现精确的时间统计。目前只是以docker运行到结束的时间为准
// TODO 解决: 7. 编译c++程序出现编译错误无法正确捕捉，现在只能设置成系统错误
//       	  考虑将6，7在c++层面去实现
