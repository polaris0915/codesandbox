...
func Init(mode string) {
	...
	// 创建日志文件写入器
	if err := os.MkdirAll(settings.LoggerConfig.LogPath, 0755); err != nil {
		panic("Failed to create log directory: " + err.Error())
	}
	logAllFile, _ := os.OpenFile(settings.LoggerConfig.LogPath+"/log_all.log", os.O_APPEND|os.O_CREATE|os.O_WRONLY, 0644)
	...
	core := zapcore.NewTee(
		...
		zapcore.NewCore(encoder, logAllWriter, allLevel),
	)
